 ...
 * The FieldCheck block class.
 */
class FieldCheck implements FormBlockInterface {
	/**
	...
	public function blockTypeMetadata() {
		return inquirywp()->basePath( '/build/block-library/field-check' );
	}
	...
	public function renderBlock( $attributes, $content ) {
		...
		$field_control = sprintf(
			'<input class="field-control" type="checkbox" id="%s" name="%s" value="" />',
			esc_attr( $field_name ),
			esc_attr( $field_name )
		);
