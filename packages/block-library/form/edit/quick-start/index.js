	...
	switch ( goal ) {
		case 'collect-info':
			innerBlocks = [
				createBlock(
					'omniform/field',
					{ fieldLabel: __( 'Your Name', 'omniform' ) },
					[
		...
		case 'generate-leads':
			innerBlocks = [
				...
				createBlock(
					'omniform/field',
					{
						fieldLabel: __( 'Business Email', 'omniform' ),
						isRequired: true,
					},
					[
						createBlock( 'omniform/label' ),
				...
				createBlock(
					'omniform/field',
					{ fieldLabel: __( 'Company Name', 'omniform' ) },
					[
						createBlock( 'omniform/label' ),
						createBlock( 'omniform/input' ),
					],
				...
				createBlock(
					'omniform/field',
					{ fieldLabel: __( 'Phone Number', 'omniform' ) },
					[
						createBlock( 'omniform/label' ),
				...
				createBlock(
					...
					[
						createBlock( 'omniform/button', {
							buttonType: 'submit',
							buttonLabel: __( 'Get in Touch', 'omniform' ),
						} ),
					],
				),
			];
		...
		case 'get-feedback':
			innerBlocks = [
				createBlock(
					'omniform/field',
					{ fieldLabel: __( 'Your Name (Optional)', 'omniform' ) },
					[
						createBlock( 'omniform/label' ),
						createBlock( 'omniform/input' ),
					],
				...
				createBlock(
					'omniform/field',
					{ fieldLabel: __( 'Your Email (Optional)', 'omniform' ) },
					[
						createBlock( 'omniform/label' ),
						createBlock( 'omniform/input' ),
					],
				...
				createBlock(
					'omniform/fieldset',
					{
						fieldLabel: __( 'Please rate our website', 'omniform' ),
						isRequired: true,
					},
					[
						createBlock(
							'omniform/field',
							{
								fieldLabel: __( '1 - Very Bad', 'omniform' ),
						...
						createBlock(
							'omniform/field',
							{
								fieldLabel: __( '3 - Average', 'omniform' ),
								className: 'is-style-inline',
							},
							[
								createBlock( 'omniform/input', {
						...
						createBlock(
							'omniform/field',
							{
								fieldLabel: __( '5 - Excellent', 'omniform' ),
								className: 'is-style-inline',
							},
							[
								createBlock( 'omniform/input', {
				...
				createBlock(
					...
					[
						createBlock( 'omniform/button', {
							buttonType: 'submit',
							buttonLabel: __( 'Send Feedback', 'omniform' ),
						} ),
					],
				),
			];
	...
	const createForm = async () => {
		...
		if ( trackPerformance ) {
			try {
				...
				blockObject.setAttributes(
					Object.keys( blockObject.attributes ).reduce(
						( acc, key ) => {
							if ( key !== 'ref' ) {
								acc[ key ] = undefined;
							}
		...
		<NavigatorProvider initialPath="/" className="omniform-quick-start">
			...
			>
				<QuickStartOption
					...
					description={ __(
						'Let us guide you through building your form',
						'omniform',
					) }
				/>
			...
			<QuickStartScreen
				...
				subtitle={ __(
					'What do you want to achieve with this form?',
					'omniform',
				) }
				showBackButton
			>
				<QuickStartOption
					...
					description={ __(
						'Gather basic details like names and emails',
						'omniform',
					) }
				/>
				...
				<QuickStartOption
					...
					description={ __(
						'Capture potential customers for follow-up',
						'omniform',
					) }
				/>
				...
				<QuickStartOption
					...
					description={ __(
						'Receive opinions or suggestions from visitors',
						'omniform',
					) }
				/>
			</QuickStartScreen>
			...
			<QuickStartScreen
				...
				subtitle={ __(
					'Do you want to monitor traffic and conversions?',
					'omniform',
				) }
				showBackButton
			>
				<QuickStartOption
					...
					title={ __(
						'Track visits and conversion rates',
						'omniform',
					) }
				/>
			...
			<QuickStartScreen
				...
				subtitle={ __(
					'Add spam protection to your form.',
					'omniform',
				) }
				showBackButton
...
const QuickStartScreen = ( {
	...
	<NavigatorScreen path={ path } className="omniform-quick-start__step">
		...
		<HStack className="omniform-quick-start__buttons">
			{ showBackButton && (
				<NavigatorBackButton
					className="omniform-quick-start__buttons-back"
					variant="tertiary"
				>
					{ __( 'Back', 'omniform' ) }
				</NavigatorBackButton>
...
const QuickStartOption = ( {
	...
	>
		<VStack alignment="center" justify="center">
			...
			{ description && (
				<div className="omniform-quick-start__option-description">
					{ description }
				</div>
			) }
		</VStack>
