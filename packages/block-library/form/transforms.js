	...
	from: [
		{
			...
			blocks: [ 'core/group' ],
			isMatch: ( attributes ) =>
				attributes?.metadata?.name === 'Sample Form',
				...
				const transformBlocks = ( blocks ) => {
					return blocks.reduce( ( transformedBlocks, block ) => {
						...
						if (
							...
							block.innerBlocks = transformBlocks(
								block.innerBlocks,
							);
						}
						...
						if (
							...
							const fieldPlaceholder =
								block.innerBlocks?.[ 0 ]?.attributes?.content?.text
									.trim()
									.replace( /\*$/, '' );
							...
							const controlType = block.attributes?.style
							...
							const prevBlock =
								transformedBlocks[
									transformedBlocks.length - 1
								];
							if (
								prevBlock &&
								prevBlock.name === 'core/paragraph'
							) {
								hasLabel = true;
								fieldLabel =
									prevBlock.attributes.content.originalHTML
										.trim()
										.replace( '*', '' );
								isRequired = prevBlock.attributes.content.text
									.trim()
									.endsWith( '*' );
						...
						if (
							...
							block.attributes.content.text ===
								'This is a sample form that you should replace with your own form solution.'
						) {
							return transformedBlocks;
						}
						...
						createBlock(
							...
							[
								...
								createBlock(
									...
									{
										callback:
											'{{omniform_comment_login_required}}',
										reverseCondition: false,
									},
									[
										createBlock( 'core/paragraph', {
