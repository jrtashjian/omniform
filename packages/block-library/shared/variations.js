...
export const createInputField = (
	...
	return {
		name: 'omniform/field',
		attributes: {
			className: [ 'checkbox', 'radio' ].includes( type )
				? 'is-style-inline'
				: '',
			fieldLabel,
			fieldName:
				extra?.fieldName || cleanFieldName( fieldLabel ).toLowerCase(),
			isRequired,
		},
...
export const createSelectBlock = (
	...
	const inputBlock = {
		...
		attributes: {
			fieldPlaceholder: __( 'Select One', 'omniform' ),
			isMultiple: false,
			fieldValue: extra?.fieldValue || '',
		},
	};
...
export const createTextareaField = ( fieldLabel, isRequired, extra = {} ) => {
	...
	const textareaBlock = {
		...
		attributes: { fieldValue: extra?.fieldValue || '' },
	};
